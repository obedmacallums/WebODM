...
- [ ] T012 [P] [US1] Crear `coreplugins/realign/public/Realign.jsx`: `L.Control` con botón que alterna el panel `RealignPanel`, calcado de `coreplugins/viewshed/public/Viewshed.jsx`, usando `public/icon.svg` (T003) y clase `leaflet-control-realign`
- [ ] T013 [P] [US1] Crear `coreplugins/realign/public/Realign.scss` con los estilos del botón del control (adaptar de `coreplugins/viewshed/public/Viewshed.scss`, clase `leaflet-control-realign`)
- [ ] T014 [US1] Crear `coreplugins/realign/public/RealignPanel.jsx` (esqueleto): componente React con props `map`/`tasks`/`isShowed`/`onClose` (mismas propTypes que `ViewshedPanel`); estado `{points:[], transform:null, error:null, mode:'idle', products:[]}`; al montar, GET `.../realign/state` para poblar `products` y, si `products` está vacío, deshabilitar la herramienta con mensaje claro (FR-001/FR-016)
- [ ] T015 [US1] En `RealignPanel.jsx`, mostrar la ortofoto semitransparente al activar la herramienta: bajar la opacidad de la capa de ortofoto localizada en `this.props.map`/`args.tiles`, y restaurarla al cerrar/desactivar (FR-002)
- [ ] T016 [US1] En `RealignPanel.jsx`, implementar la captura de pares de puntos con dos clics por par: primer clic (origen sobre la ortofoto) y segundo clic (destino sobre el mapa base) vía `this.props.map.on('click', ...)`; renderizar marcadores origen/destino y un vector que los une; soportar añadir, mover (drag del marcador) y eliminar pares, actualizando `this.state.points`; limpiar listeners en `componentWillUnmount` (FR-003/FR-004)
- [ ] T017 [US1] En `RealignPanel.jsx`, recalcular la transformación con `similarity.js` ante cualquier cambio de puntos y renderizar una tabla con el residuo por punto y el RMSE global, actualizada de forma percibida como inmediata (< 1 s, SC-002) (FR-005/FR-006)
- [ ] T018 [US1] En `RealignPanel.jsx`, implementar la previsualización: derivar una matriz CSS afín (`transform: matrix(...)`) de la similitud en el espacio de píxeles al zoom actual y aplicarla al contenedor/`pane` de las capas ráster (ortofoto+DSM+DTM) de `args.tiles`, moviéndolas **juntas**; recalcular la matriz en `zoomend`/`moveend`; quitar el `transform` al cerrar/desactivar (FR-007, D3 de research.md — pieza de mayor riesgo, validar primero)
- [ ] T019 [US1] En `RealignPanel.jsx`, detectar en cliente puntos insuficientes/ajuste degenerado (usando `degenerate` de `similarity.js`) y mostrar el motivo, anticipando el bloqueo de "Aplicar" (espejo de FR-015; refuerza la validación server-side que llega en US2)
...
- [ ] T021 [US1] Validación manual de quickstart.md sección 2, escenarios 1–5 y 12 (activación, marcar pares, traslación con 1 par, similitud con 2–4 pares, recálculo al mover/eliminar, tarea sin productos 2D)
...
- [ ] T032 [US2] Implementar GET `.../realign/download/<type>` (`RealignDownload`): entregar el `.tif` corregido desde el dir. persistente con `download_file_response` (patrón de `app/api/tasks.py`); 404 si no aplicado
...
- [ ] T038 [US3] En `RealignPanel.jsx`, agregar el botón "Revertir" (con `change_project`): si hay corregidos aplicados, POST `revert` y restaurar las capas del core (quitar el `L.tileLayer` del plugin); si es solo previsualización, descartar puntos y quitar la matriz CSS; en ambos casos volver al estado original y refrescar la UI (FR-011)
- [ ] T039 [US3] Ejecutar los tests de US3 en Docker (exit 0, salida visible) y validar manualmente quickstart.md escenario 10 (revertir tras aplicar)
...
**Purpose**: cierre de calidad transversal a las cuatro historias.
...
- [ ] T045 Ejecutar el checklist funcional completo de `specs/002-realign-products/quickstart.md` sección 2 (los 12 escenarios) contra el stack en Docker y registrar el resultado de cada uno
...
- [ ] T047 Ejecutar de nuevo `./run_tests_in_docker.sh backend coreplugins.realign.tests` (+ `... test frontend` para `similarity.js`) tras todos los cambios y confirmar exit code 0 (regla `verification-before-completion`, cierre de feature), incluida la verificación de worker de T035
...
- Tests antes de la implementación (T008–T009 antes de T010–T019; T022–T024 antes de T025–T034; T036 antes de T037–T038; T040 antes de T041–T042)
...
6. + Polish → checklist completo de quickstart.md y cierre de verificación
