...
- [ ] T060 Ejecutar la suite completa y mostrar su salida: `./run_tests_in_docker.sh`, más `webodm.sh test backend coreplugins.realign.tests` para confirmar que la ampliación de `realign` no rompió nada — **bloqueado por upstream**: el submódulo `nodeodm/external/NodeODM` no está inicializado y su URL en `.gitmodules` apunta a `WebODM/NodeODX`, que no existe, así que `docker compose build` falla en `./nodeodm/setup.sh` para cualquier rama. Verificado en su lugar contra el stack vivo: `road` 200, `realign` 60 y `annotations` 66, todas exit 0; los 16 fallos del core (15 por NodeODM ausente, 1 por versión de PROJ en `test_task_wkt`, que falla igual en aislamiento) son previos y ninguno está en un archivo que esta rama toque
